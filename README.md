...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; transition: 0.4s; font-family: 'Poppins', 'Segoe UI', sans-serif; }
...
<body>
    ...
    <script>
        ...
        // Trigger initial fade-in for top items
        ...
        function playMusic(id, btn) {
            ...
            document.querySelectorAll('audio').forEach(m => {
                if(m.id !== id) {
                    m.pause();
                    let b = m.nextElementSibling;
                    b.innerText = "▶ Play Music " + m.id.replace('audio', '');
                }
            });
            if (audio.paused) {
                audio.play();
